...
class PlannerModelConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.deliberation_steps <= 0:
            raise ValueError("model.deliberation_steps must be positive")
        if self.memory_slots < 0:
            raise ValueError("model.memory_slots must be non-negative")
        if self.architecture == "recurrent_v1":
            if self.deliberation_steps < 2:
                raise ValueError(
                    "model.deliberation_steps must be at least 2 for architecture 'recurrent_v1'"
                )
            if self.memory_slots <= 0:
                raise ValueError(
                    "model.memory_slots must be positive for architecture 'recurrent_v1'"
                )
        if not 0.0 <= self.dropout < 1.0:
            raise ValueError("model.dropout must be in [0.0, 1.0)")
...
class PlannerOptimizationConfig:
    ...
    teacher_rank_loss_weight: float = 0.0
    curriculum_priority_weight: float = 0.0
    ...
    def __post_init__(self) -> None:
        ...
        if self.teacher_rank_loss_weight < 0.0:
            raise ValueError("optimization.teacher_rank_loss_weight must be non-negative")
        if self.curriculum_priority_weight < 0.0:
            raise ValueError("optimization.curriculum_priority_weight must be non-negative")
        if self.root_value_loss_weight < 0.0:
            raise ValueError("optimization.root_value_loss_weight must be non-negative")
        if self.root_gap_loss_weight < 0.0:
            raise ValueError("optimization.root_gap_loss_weight must be non-negative")
