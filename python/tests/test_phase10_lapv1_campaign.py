...
def test_build_resolved_arena_spec_can_use_resolved_lapv1_agent_path(tmp_path: Path) -> None:
    arena_path = tmp_path / "arena.json"
    verify_path = tmp_path / "verify.json"
    suite_path = tmp_path / "initial_fens.json"
    ...
    resolved = _build_resolved_arena_spec(
        spec,
        ["arm_a"],
        lapv1_agent_path=resolved_agent_path,
    )
    assert resolved.agent_specs["lapv1_stage1_all_unique_v1"] == str(resolved_agent_path)
