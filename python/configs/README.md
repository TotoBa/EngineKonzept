...
`lapv2.shared_opponent_readout`, and `lapv2.accumulator_cache` are live;
...
targets. `lapv2.accumulator_cache` is eval/runtime-only: it keeps the
existing vectorized training path, but the NNUE policy scorer may reuse
fixed-phase incremental successor accumulators in inference and now
writes cache hit/miss diagnostics into the model outputs.
The first explicit LAPv2 warm-start export path now lives in
[build_lapv2_warm_start_checkpoint.py](/home/torsten/EngineKonzept/python/scripts/build_lapv2_warm_start_checkpoint.py).
It takes one existing LAPv1 stage-T2 checkpoint plus a target LAPv2
training config and materializes an init checkpoint for the next run:
phase-routed encoder/embedder/sharpness modules are expanded from the
source weights, while `ft`, `value_head_nnue`, `policy_head_nnue`, and
`opponent_readout` remain freshly initialized.
...
- [phase10_lapv1_stage2_fast_all_unique_v2.json](/home/torsten/EngineKonzept/python/configs/phase10_lapv1_stage2_fast_all_unique_v2.json)
  Preferred current Stage-T2 config. It keeps the fast all-unique Stage-T1 warm start and the same precomputed `lapv1_*` workflow artifacts, but now uses explicit Stage-T2 phases: `freeze_inner` for `2` epochs with only the inner-loop path trainable and a `1 -> 2` step schedule, followed by `joint_finetune` for `2` epochs with the whole wrapper reopened and a `2 -> 4` schedule. This config is aligned with per-example halting/rollback, residual reranking over root logits, root-vs-final diagnostics, and the new improvement-over-root supervision path that teaches final and intermediate step logits to beat the detached root policy on rows where the root is still wrong. Stage-T2 phases may now also override their own train/validation artifact paths and schedule both `min_inner_steps` and `max_inner_steps`, which is the mechanism used for the next hard-position curriculum runs.
