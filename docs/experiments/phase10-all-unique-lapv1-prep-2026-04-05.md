...
- larger warm-start batch size on this host (`48` instead of `12`) because the
  current fast trainer sits near `~1.2 GiB RSS` at batch `12` and leaves ample
  memory headroom on a `23 GiB` machine
