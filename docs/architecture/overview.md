...
The repository now also has prepared-but-untrained LAPv1 scaffolding as the next
unification target for the planner family:

- versioned `StateContextV1` on both Python and Rust sides
- piece-intention encoder and relational state embedder references
- large value and candidate-policy heads
- bounded recurrent deliberation core with rollback-aware traces
- LAPv1 wrapper, staged trainer support, runtime glue, and a prepared arena template

This does not replace the current planner family yet. LAPv1 remains a migration
target until it beats the retained references empirically. The current target
architecture and migration pressure are documented in
[lapv1-overview.md](/home/torsten/EngineKonzept/docs/architecture/lapv1-overview.md)
and
